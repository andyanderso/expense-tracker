...
**Reports.** Two independent generators share the same data (`reportSet()`) and column layout (colgroup widths, "Card charge (USD)" column omitted when no receipt in the set has a `billedAmount`), but are otherwise separate code paths — keep both in sync when changing report content. `reportHTML()` builds Word-flavored HTML for `docBtn`/`printBtn` (local download / print), receipt images capped to `max-width:300px;max-height:480pt` with `page-break-inside:avoid` so one receipt doesn't spill across pages. `buildReportDocx()` (used only by "Save to Drive" / `pickSaveBtn`) builds a genuine OOXML `.docx` client-side with the `docx` library (`docx.Document`/`Table`/`ImageRun`/`Packer.toBlob`), embedding images as real media parts sized via `fitImageSize()` (capped to `REPORT_IMG_MAX_W`/`REPORT_IMG_MAX_H` px, aspect-preserving, read via a throwaway `Image()` load) — this was necessary because Drive's Google-Doc conversion was silently dropping inline `data:` URI images from both an `application/msword`-labeled upload and a genuine `text/html` upload; converting a real `.docx` (Drive's best-supported import format) fixed it. The upload multipart body is built as a `Blob` (not a string) since the file part is now binary. The in-app folder browser starts at the configured sync folder.

**External libraries** (pdf.js 3.11.174, Google Identity Services, `docx` 9.7.1 via its `index.iife.js` browser build — global `window.docx`) load lazily from CDNs via `loadScript()` — nothing is vendored. The service worker only caches same-origin GETs and must never intercept cross-origin API calls.
