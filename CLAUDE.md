...
**Reports.** `reportHTML()` builds Word-flavored HTML (summary by type, itemized table — `<colgroup>`-driven column widths, "Card charge (USD)" column omitted when no receipt in the set has a `billedAmount` — then each receipt image page-broken with its statement screenshot beneath). `docBtn`/`printBtn` (local download / print) still wrap it as legacy Word-flavored HTML (`application/msword` blob). "Save to Drive" (`pickSaveBtn`) uploads it as genuine `text/html` (not `application/msword`) with metadata `mimeType: 'application/vnd.google-apps.document'` so Drive converts it to a native Google Doc — real `text/html` was found to preserve inline `data:` URI receipt images through Drive's importer better than the `application/msword`-labeled upload did; the in-app folder browser starts at the configured sync folder.
